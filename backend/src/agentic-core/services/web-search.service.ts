  ...
  async executeAgenticSearch(
          ...
          contents: [
            {
              ...
              parts: [
                {
                  text: `Lakukan pencarian Google yang mendalam, teliti, dan akurat untuk sub-task berikut: "${currentSubQuery}". Sertakan fakta-fakta spesifik, data teknis, tanggal/versi terbaru, dan rujukan dokumen primer.`,
                },
              ],
            },
          ],
    ...
    const compiledEvidence =
      groundingEvidenceBlocks.length > 0
        ...
        : `Tidak ditemukan hasil pencarian spesifik untuk: "${query}". Gunakan pengetahuan berbasis parameter yang terverifikasi.`;
    ...
    if (
      ...
      const bulletPoints = topEvidence
        .map(
          (s) =>
            `- **${s.title}**: ${s.key_evidence || s.relevance} [${s.source_type || 'Sumber'}](${s.url})`,
        )
        .join('\n');

      synthesizedMarkdown = `### 🔍 Ringkasan Hasil Penelusuran & Fakta Terkini\n\nBerdasarkan data dan perkembangan terkini mengenai **"${query}"**, berikut rangkuman informasi utama yang berhasil diverifikasi:\n\n${bulletPoints}`;
    }
